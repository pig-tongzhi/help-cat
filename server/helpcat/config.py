...
class Settings:
    def __init__(self, database_url=None, storage_root=None, fake_admin_openids=None):
        ...
        # 「记住这台设备」的有效期。默认与常规会话一致（30 天）：
        # 两者的区别不在"活多久"，而在**存在哪** —— 记住设备写 localStorage/HttpOnly Cookie
        # （关浏览器、微信清 JS 存储都还在），不记住只写 sessionStorage（关标签页即失效）。
        self.session_days_remember = int(os.getenv("HELPCAT_SESSION_DAYS_REMEMBER", "30"))
