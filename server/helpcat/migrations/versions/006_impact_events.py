...
def upgrade():
    if not sa.inspect(op.get_bind()).has_table("impact_events"):
        op.create_table(
            "impact_events",
        ...
        op.create_index("ix_impact_events_created_by", "impact_events", ["created_by"])


def downgrade():
    op.drop_index("ix_impact_events_created_by", table_name="impact_events")
