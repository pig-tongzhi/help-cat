...
发布任务在完成数据库备份、确认目标 `community_id` 和已批准公开肖像后，可使用幂等导入脚本：
...
脚本只从指定的环境变量读取密码，输出仅包含 `cat_id`、`media_id` 和是否发生变更。它以 `2025-06-02 相遇` 为稳定标记检索公开与管理员档案：档案不存在时上传一次已批准肖像并创建，已存在时不重复上传或创建；待审核或隐藏的既有档案会由 `ADMIN`/`SUPER_ADMIN` 审核并恢复公开。导入字段明确保持未知健康状态，不推断性别、疫苗、绝育、健康细节、精确地址或坐标。
