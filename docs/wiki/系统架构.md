...
| 备份 | `scripts/backup_offsite.sh`：WAL 安全快照 + 自检 + 保留期 + 异地钩子；systemd 每日 03:20；`scripts/pull_backup.sh` 在开发机上拉第二份 | 真正的异地仍待配置（只有一台云机器） |
| 可观测性 | `/api/v1/health`（liveness）+ `/api/v1/health/ready`（真查库）+ `scripts/health_check.sh` + 5 分钟 systemd timer + 告警 webhook；`.github/workflows/uptime.yml` 每 15 分钟从机器外探一次；journald 与 nginx 日志有轮转 | 机器外的探针已就位，且不需要额外账号 |
