...
## 架构评估与优化路线（2026-09-23 实测）

### 实测现状

| 维度 | 实测 | 判断 |
|---|---|---|
| 后端单体 | `app.py` **1780 行**、57 条路由、92 个函数 | 已是瓶颈：任何改动都要在一个文件里定位 |
| 前端 | 无构建链，`rescue/app.js` 1707 行 + `styles.css` 1072 行、`admin/app.js` 930 行 | 仍可用，但接近上限 |
| 数据 | SQLite，`journal_mode=delete`（**未开 WAL**），无 `busy_timeout` | 单写者、读写互斥 |
| Schema 来源 | Alembic（11 个迁移）**+ `ensure_schema` 里 23 条裸 ALTER** | 两个真相来源，容易漂移 |
| 图片 | **经 FastAPI 读文件返回**（`GET /api/v1/media/{id}`） | 每张图都占用一个 Python worker |
| 并发 | uvicorn **单 worker**（ExecStart 无 `--workers`） | 图片 I/O 与 API 抢同一个进程 |
| 静态缓存 | 线上 CSS / 图片 **0 个 `Cache-Control`** | 只能靠改 URL 失效（因此必须升版本号） |
| 应用状态 | `app.state` 只有 settings / session_factory / provider；会话、配额、限流都在库里 | **请求处理无进程内状态 → 可以安全加 worker** |
| CI | **没有** `.github/workflows` | 318 个测试只在本地手动跑 |
| 备份 | 同盘目录拷贝 | 磁盘故障即全丢 |
| 可观测性 | 只有 journalctl | 无指标、无告警、无外部探活 |

### 优化路线（按 收益/成本 排序）

**P0 —— 几乎不动业务代码就能拿到收益**

1. **图片改由 nginx 直出**：FastAPI 只做鉴权并返回 `X-Accel-Redirect`，文件由 nginx 读。单 worker 不再被图片 I/O 占满，这是吞吐量最大的一笔。
2. **补 `Cache-Control`**：版本化资源 `immutable`、HTML `no-cache`、图片长缓存。现在完全缺失。
3. **SQLite 开 WAL** + `busy_timeout=5000` + `synchronous=NORMAL`；这是第 4 条的前提。
4. **uvicorn 提到 2–4 workers**（第 3 条完成后；已确认应用无进程内状态）。
5. **加 CI**：push 即跑 pytest。测试已经写好，缺的只是自动化。

**P1 —— 结构性**

6. 拆 `app.py`：按领域拆 routers（cats / feeding / tasks / admin / media），业务规则下沉到 service 层。
7. Schema 单一来源：把 `ensure_schema` 的 23 条 ALTER 收敛进迁移，`ensure_schema` 只留旧库兜底。
8. 异地备份：每日 dump 推对象存储（现在只有同盘拷贝）。
9. 探活与告警：外部 uptime 监控 + 日志轮转。
10. 版本号自动升（`scripts/bump_version.sh`）：漏升会让已访问用户拿不到新版。

**P2 —— 规模上来再说**

11. 媒体放对象存储 + CDN。
12. 迁移到 PostgreSQL（真出现并发写压力时；当前规模 SQLite + WAL 足够）。
13. 前端引入构建链（`app.js` 继续增长时）。

### 当前容量边界

- 单 worker + 图片过 Python：并发图片请求会排队（首页一次加载多张缩略图即可触发）
- SQLite 单写者：写入串行；开 WAL 后读不再被写阻塞
- 全部组件在一台 VM 上，无冗余；磁盘故障同时丢数据与站点
