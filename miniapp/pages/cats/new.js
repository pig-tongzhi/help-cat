...
Page({
  ...
  uploadPhoto(path) { return compressPhoto(path).then((compressedPath) => api.uploadImage(compressedPath)).then((asset) => this.setData({ photoAssetId: asset.id, uploading: false, uploadError: false })).catch(() => { this.setData({ uploading: false, uploadError: true }); wx.showToast({ title: "照片上传失败", icon: "none" }); }); },
  choosePhoto() { api.ensureLogin().then(() => new Promise((resolve) => wx.chooseMedia({ count: 1, mediaType: ["image"], sourceType: ["album", "camera"], success: resolve, fail: () => resolve(null) }))).then((res) => { if (!res) return; const path = res.tempFiles[0].tempFilePath; this.setData({ photoPath: path, photoAssetId: "", uploading: true, uploadError: false }); return this.uploadPhoto(path); }).catch(() => wx.showToast({ title: "请先完成登录", icon: "none" })); },
  retryPhoto() { const path = this.data.photoPath; if (!path) return; this.setData({ photoAssetId: "", uploading: true, uploadError: false }); this.uploadPhoto(path); },
